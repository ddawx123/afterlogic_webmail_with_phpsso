...
require_once(dirname(__FILE__).'/encrypt.class.php');
...
class Common {
     ...
     * @param string $usermail 邮箱地址
     * @param string $password 邮箱密码
     * @return boolean 绑定结果
     */
    public function bindUserInfo($sqlhandle, $username, $usermail, $password = '') {
        $result = $sqlhandle->query('select * from mailusers where username="'.$username.'"');
        $data = $result->fetch_all(MYSQLI_ASSOC);
        if (isset($data['usermail'])) {
            $result = $sqlhandle->query('update mailusers set password="'.$password.'" where username="'.$username.'"');
            if ($result) {
                return true;
            }
            else {
            ...
            $result = $sqlhandle->query('insert into mailusers (username, usermail, password) values ("'.$username.'", "'.$usermail.'", "'.$password.'")');
            if ($result) {
                return true;
            }
            else {
